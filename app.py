    ...
    if file and file.filename.endswith(('.xls', '.xlsx')):
        try:
           ...
           # Filter data based on the selected status
            ...
            zoracle_count = len(data[data['Action Item'].str.contains('Oracle')])
            rehydration_count = len(data[data['Action Item'].str.contains('Rehydrate')])
            ssh_count = len(data[data['Action Item'].str.contains('SSH')])
            microsoft_count = len(data[data['Action Item'].str.contains('Microsoft')])

            past_due_total = len(data[data['PD Flag'] == 'Past Due'])
            past_due_high = len(data[(data['PD Flag'] == 'Past Due') & (data['Risk Level'] == 'High/Critical')])
            past_due_medium = len(data[(data['PD Flag'] == 'Past Due') & (data['Risk Level'] == 'Medium')])
            past_due_low = len(data[(data['PD Flag'] == 'Past Due') & (data['Risk Level'] == 'Low')])
            past_due_without_exception = len(data[(data['PD Flag'] == 'Past Due') & (data['Risk Level'] == 'None')])

            not_past_due_total = len(data[data['PD Flag'] != 'Past Due'])
            not_past_due_high = len(data[(data['PD Flag'] != 'Past Due') & (data['Risk Level'] == 'High/Critical')])
            not_past_due_medium = len(data[(data['PD Flag'] != 'Past Due') & (data['Risk Level'] == 'Medium')])
            not_past_due_low = len(data[(data['PD Flag'] != 'Past Due') & (data['Risk Level'] == 'Low')])
            not_past_due_without_exception = len(data[(data['PD Flag'] != 'Past Due') & (data['Risk Level'] == 'None')])

            summary_report = {
                'total_vulns': total_vulns,
                'zoracle_count': zoracle_count,
                'rehydration_count': rehydration_count,
                'ssh_count': ssh_count,
                'microsoft_count': microsoft_count,
                'past_due_total': past_due_total,
                'past_due_high': past_due_high,
                'past_due_medium': past_due_medium,
                'past_due_low': past_due_low,
                'past_due_without_exception': past_due_without_exception,
                'not_past_due_total': not_past_due_total,
                'not_past_due_high': not_past_due_high,
                'not_past_due_medium': not_past_due_medium,
                'not_past_due_low': not_past_due_low,
                'not_past_due_without_exception': not_past_due_without_exception
            }

            return render_template('template.html', data=output, summary_data=due_dates_summary, summary_report=summary_report)
        except Exception as e:
            return f'Error reading Excel file: {str(e)}', 500
    else:
        return 'Invalid file format. Please upload an Excel file.', 400
